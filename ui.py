...
MODEL_BADGE = "HLASOVÉ JÁDRO · Windows"
...
# používáme samostatnou rodinu Extra Bold.
...
def _czech_date_text(now: time.struct_time) -> str:
    return f"{now.tm_mday}. {CZECH_MONTHS[now.tm_mon]} {now.tm_year}"


def _czech_weekday_text(now: time.struct_time) -> str:
    return CZECH_WEEKDAYS[now.tm_wday]
...
class JarvisUI:
    def __init__(self):
        ...
        # Správce oken někdy přepíše geometrii, proto ji znovu vynutíme.
        for delay in (80, 220, 600, 1200):
            self.root.after(delay, self._force_startup_size)
        # Po několika sekundách vypni topmost a obnov běžné chování.
        ...
        self.tick            = 0
        self.rings_spin      = [0.0, 45.0, 90.0, 200.0]  # čtyři samostatné kruhy
        self.pulse_r         = []
        ...
        self._weather_card = {
            "city": "Praha",
            "primary": "--",
            "details": ["Načítám počasí..."],
        }
        self._health_card_lines = ["Načítám zdravotní data..."]
        self._panel_focus = ""
        ...
        # Kliknutí na orb pozastaví nebo obnoví běh.
    ...
    def _draw_shutdown_button(self):
        ...
        c.create_text(BW//2, BH//2, text="⏻  UKONČIT",
    ...
    def _draw_settings_button(self):
        ...
        c.create_text(14, 15, text="SYSTÉMOVÁ NASTAVENÍ", fill=C_PRI, font=font_display(10), anchor="w")
    ...
    def _draw_settings_tabs(self):
        for key, canvas, label in (
            ("settings", self._settings_tab_settings, "NASTAVENÍ"),
            ("debug", self._settings_tab_debug, "LADĚNÍ"),
        ):
    ...
    def _refresh_settings_status(self):
        ...
        primary = [
            "Gemini připraveno" if gemini_ready else "Chybí Gemini API",
            "YouTube připraveno" if yt_key_ready and yt_handle else "Chybí nastavení YouTube",
        ]
        if yt_handle:
            handle_text = yt_handle
        else:
            handle_text = "@handle nebyl zadán"
        secondary = f"Kanál: {handle_text}"
    ...
    def _render_debug_logs(self):
        ...
        if not self._debug_entries:
            self._debug_text.insert(tk.END, "Zatím nebyla zaznamenána žádná chyba.\n", "info")
        else:
    ...
    def _draw_api_button(self):
        ...
        c.create_text(bw // 2, bh // 2, text="⌘ NASTAVENÍ API",
    ...
    def _build_fx_slider(self, parent=None):
        ...
        self._volume_label = tk.Label(
            parent,
            text=f"ÚROVEŇ EFEKTŮ  {int(self.sound.get_volume() * 100)}%",
            fg=C_PRI,
            bg=parent.cget("bg"),
    ...
    def _on_volume_change(self, value):
        try:
            ...
            return
        self._volume_label.configure(text=f"ÚROVEŇ EFEKTŮ  {volume}%")
    ...
    def _draw_sfx_button(self):
        ...
        text = "♪ ZVUKY ZAP" if self._sfx_on else "♪ ZVUKY VYP"
        bl = 6
        for bx, by, sx, sy in [(0, 0, 1, 1), (BW, 0, -1, 1),
                                (0, BH, 1, -1), (BW, BH, -1, -1)]:
    ...
    def _draw_mute_button(self):
        ...
        c.delete("all")
        if self.muted:
            col, icon, lbl = C_MUTED, "🔇", " ZTLUMENO"
        else:
            col, icon, lbl = C_GREEN, "🎙", " AKTIVNÍ"
        bl = 6
        for bx, by, sx, sy in [(0, 0, 1, 1), (bw, 0, -1, 1),
                                (0, bh, 1, -1), (bw, bh, -1, -1)]:
    ...
    def _draw_pause_button(self):
        ...
        c.delete("all")
        if self.paused:
            col, text = C_GOLD, "▶ POKRAČOVAT"
        else:
            col, text = C_BLUE, "⏸ POZASTAVIT"
        bl = 6
        for bx, by, sx, sy in [(0, 0, 1, 1), (bw, 0, -1, 1),
                               (0, bh, 1, -1), (bw, bh, -1, -1)]:
            c.create_line(bx, by, bx+sx*bl, by, fill=col, width=2)
    ...
    def _toggle_mute(self):
        ...
        self._draw_mute_button()
        if self.muted:
            self.write_log("SYS: Mikrofon byl vypnut.")
        else:
            self.write_log("SYS: Mikrofon je zapnutý.")
        self._sync_sound_state()

    # ── Kliknutí na orb = pozastavení ────────────────────────────────────────
    ...
    def _toggle_pause(self):
        ...
        if self.paused:
            ...
            self.write_log("SYS: JARVIS byl pozastaven.")
        else:
            self.set_state("THINKING")
            self.write_log("SYS: JARVIS pokračuje...")
        self._sync_sound_state()
    ...
    def _shutdown(self):
        self.sound.stop_all()
        self.write_log("SYS: JARVIS se ukončuje...")
    ...
    def _on_input_submit(self, event=None):
        ...
        if not text:
            return
        if self.paused:
            self.write_log("SYS: JARVIS je pozastaven. Pro pokračování vypněte pozastavení.")
            return
        self._input_var.set("")
        if text.lower() in ("ticho", "zastav", "stop", "ztlum", "přestaň"):
            self.write_log("SYS: ⏹ Přehrávání bylo zastaveno.")
    ...
    def wake_up(self):
        """Dvojité tlesknutí přenese okno do popředí."""
        def _do():
            self.root.deiconify()
            self.root.attributes("-topmost", True)
            self.root.lift()
    ...
    def _state_display_text(state: str) -> str:
        return STATE_DISPLAY_TEXT.get(state, state)

    # ── Log ──────────────────────────────────────────────────────────────────
    def write_log(self, text: str):
        self.typing_queue.append(text)
        tl = text.lower()
        if tl.startswith("vy:") or tl.startswith("you:"):
    ...
    def _parse_weather_card(self, text: str) -> dict:
        if not text or "není dostup" in text.lower() or "nepodařilo" in text.lower():
            return {
                "city": "Praha",
                "primary": "--",
                "details": ["Počasí se nepodařilo načíst."],
            }

        prefix, _, body = text.partition(":")
        city = "Praha"
        if prefix.lower().startswith("počasí pro "):
            city = prefix[len("Počasí pro "):].strip().title()

        details = [part.strip(" .") for part in body.split(",") if part.strip()]
        primary = "--"
        if details:
            primary = details[0].replace(" stupňů", " °C")
        return {
            "city": city,
            "primary": primary,
            "details": details[1:4] or ["Aktuální data jsou připravena."],
        }

    def _parse_health_card(self, text: str) -> list[str]:
        if not text or "není dostup" in text.lower() or "nepodařilo" in text.lower():
            return ["Zdravotní data se nepodařilo načíst."]
        lines = self._split_summary_lines(text, limit=4)
        return lines or ["Zdravotní souhrn není připraven."]
    ...
    def _refresh_brief_cards(self):
        try:
            weather = get_weather_summary("Praha")
    ...
    # ── Zdravotní překryv v levém panelu ─────────────────────────────────────
    ...
    def _draw_health_overlay(self, c):
        ...
        c.create_text(x0+pw//2, y0+18, text="◈ ZDRAVÍ ◈",
    ...
    def _draw_left_panel(self, c):
        ...
        cards = [
            ("time", 0.22, "ČAS", C_GOLD),
            ("weather", 0.20, "POČASÍ · PRAHA", C_BLUE),
            ("system", 0.28, "STAV SYSTÉMU", C_PRI),
            ("health", 0.30, "ZDRAVOTNÍ SOUHRN", C_GREEN),
        ]
        ...
        for (section, _, title, accent), ph in zip(cards, heights):
            ...
            if section == "time":
                ...
                c.create_text(section_x+section_pad, current_y+118, text=_czech_date_text(now_local),
                              fill=muted_gold, font=font_body_bold(11), anchor="w")
                c.create_text(section_x+section_pad, current_y+138, text=_czech_weekday_text(now_local),
                ...
                c.create_text(section_x+section_pad, cy, text=f"DOBA BĚHU  {up_hr:02d}:{up_min:02d}:{up_sec:02d}",
                              fill=muted_label, font=font_body_bold(9), anchor="w")
                cy += 22
                for label, key, unit in [("CPU", "cpu", "%"), ("RAM", "ram", "%"), ("DISK", "disk", "%"), ("BATERIE", "battery", "%")]:
                    val = self._stats[key]
    ...
    def _draw_right_panel(self, c):
        ...
        c.create_text(x0+14, y0+16, text="KONVERZACE", fill=C_PRI,
    ...
    def _draw(self):
        ...
        # Pomalá a velmi jemná skenovací čára.
        ...
        # Spodní tyrkysová světelná čára.
        ...
        c.create_text(W//2, 52, text="Velmi inteligentní asistenční systém",
                      fill=C_MID, font=font_body(11))

        # Vlevo: označení modelu.
        c.create_text(22, 36, text=MODEL_BADGE,
                      fill=C_DIM, font=font_body(10), anchor="w")

        # Vpravo: indikátor stavu.
        ...
        c.create_text(W//2, H-13, fill=C_DIM, font=font_body(9),
                      text="JARVIS · Edice pro Windows · Jádro v reálném čase")
        c.create_text(W-18, H-13, fill=C_DIM, font=font_body(9),
                      text="[F4] ZTLUMIT  [F5] POZASTAVIT  [ESC] UKONČIT", anchor="e")
    ...
    def _show_setup_ui(self, edit_mode: bool = False):
        ...
        title = "◈ NASTAVENÍ API" if edit_mode else "◈ JE NUTNÉ PRVNÍ NASTAVENÍ"
        subtitle = (
            "Aktualizujte nastavení Gemini a YouTube."
            if edit_mode else
            "Zadejte klíč Gemini API. Pole YouTube jsou volitelná."
        )
        ...
        tk.Label(self.setup_frame, text="KLÍČ GEMINI API",
        ...
        tk.Label(self.setup_frame, text="KLÍČ YOUTUBE API",
        ...
        tk.Label(self.setup_frame, text="YOUTUBE HANDLE / KANÁL",
                 fg=C_DIM, bg="#00080d", font=font_body(12)).pack(pady=(10, 4))
    ...
    def _save_api_key(self):
        ...
        self._refresh_settings_status()
        if was_ready:
            self.write_log("SYS: Nastavení API bylo aktualizováno.")
        else:
            self.set_state("LISTENING")
            self.write_log("SYS: JARVIS je připraven. Poslouchám...")

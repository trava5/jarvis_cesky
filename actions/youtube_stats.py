...
def _api_get(endpoint: str, params: dict, api_key: str) -> dict:
    ...
    if reason == "keyInvalid":
        raise RuntimeError("Klíč YouTube API je pravděpodobně neplatný.")
    if reason == "quotaExceeded":
        raise RuntimeError("Kvóta YouTube API je pravděpodobně vyčerpaná.")
    if reason in {"accessNotConfigured", "forbidden"}:
        raise RuntimeError("YouTube Data API není pro tento klíč povoleno nebo je přístup zablokován.")
    if response.status_code == 404:
        raise RuntimeError("Data YouTube nebyla nalezena.")
    raise RuntimeError(message or f"Chyba YouTube API ({response.status_code}).")
...
def _fetch_channel_payload(channel_ref: str, api_key: str) -> tuple[dict, str]:
    ...
    if not filter_name or not filter_value:
        raise RuntimeError("Handle kanálu YouTube není nastaven. Zadejte @handle v nastavení.")
    ...
    if not items:
        raise RuntimeError(
            f"Kanál YouTube se nepodařilo najít. Do pole handle kanálu zadejte hodnotu jako '{filter_value}'."
        )
...
def _trend_sentence(videos: list[dict]) -> str:
    ...
    if ratio >= 1.18:
        return "Poslední videa si vedou lépe než předchozí skupina."
    if ratio <= 0.82:
        return "Poslední videa rostou o něco pomaleji než předchozí skupina."
    return "Výkon posledních videí je celkově vyrovnaný."
...
def get_youtube_channel_report(query: str = "overview", handle: str = "", video_limit: int = DEFAULT_VIDEO_LIMIT) -> str:
    ...
    if not api_key:
        return (
            "Pro statistiky YouTube je nutný klíč YouTube API. "
            "Zadejte jej v nabídce Nastavení > Nastavení API."
        )
    ...
    try:
        ...
        channel_title = snippet.get("title", "váš kanál YouTube")
        ...
        parts = [
            (
                f"Podle veřejných dat YouTube má kanál {channel_title} "
                f"{_format_int(subscribers)} odběratelů, celkem {_format_int(total_views)} zhlédnutí "
                f"a {_format_int(video_count)} videí."
            )
        ]
        if display_handle:
            parts.append(f"Označení kanálu: {display_handle}.")
        ...
        if valid_videos:
            ...
            parts.append(
                f"Posledních {len(valid_videos)} videí má průměrně {_format_int(avg_views)} zhlédnutí, "
                f"{_format_int(avg_likes)} označení To se mi líbí a {_format_int(avg_comments)} komentářů."
            )
            ...
            parts.append(
                f"Nejúspěšnější poslední video je '{best_video.get('title', 'Video')}' "
                f"- {_format_int(best_video.get('views', 0))} zhlédnutí"
            ...
            if len(publish_dates) >= 2:
                ...
                if gaps:
                    ...
                    parts.append(f"V poslední době vychází průměrně jedno video každých {avg_gap:.1f} dne.")
            ...
            if any(word in query_l for word in (
                "detail", "analysis", "report", "recent video", "recent videos",
                "podrob", "analý", "analy", "přehled", "poslední video", "poslední videa",
            )):
                recent_lines = []
                for index, video in enumerate(valid_videos[: min(3, len(valid_videos))], start=1):
                    ...
                    recent_lines.append(
                        ...
                        f"{_format_int(video.get('views', 0))} zhlédnutí, "
                        f"{_format_int(video.get('likes', 0))} označení To se mi líbí, "
                        f"{_format_int(video.get('comments', 0))} komentářů"
                        + (f" ({tail})" if tail else "")
                    )
                if recent_lines:
                    parts.append("Podrobnosti posledních videí: " + " | ".join(recent_lines) + ".")

        parts.append(
            "Poznámka: bez přístupu do YouTube Studia nelze zobrazit dobu sledování, CTR, imprese, příjmy ani zdroje návštěvnosti."
        )
        return " ".join(parts)
    except Exception as exc:
        return f"Statistiky YouTube se nepodařilo načíst: {exc}"

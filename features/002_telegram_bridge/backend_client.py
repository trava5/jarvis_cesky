"""Backend API client for Telegram bridge migration."""
...
DEFAULT_FALLBACK_STATUSES = frozenset({"runtime_unavailable", "not_implemented", "runtime_error"})


class TelegramBackendError(RuntimeError):
    """Expected backend client error that should fall back to desktop runtime."""
...
@dataclass(frozen=True)
class TelegramBackendMessageResult:
    status: str
    text: str
...
def _default_base_url() -> str:
    explicit = (
        os.getenv("TELEGRAM_BACKEND_BASE_URL")
...
def load_config() -> TelegramBackendConfig:
    return TelegramBackendConfig(
        enabled=_parse_bool(os.getenv("TELEGRAM_BACKEND_ENABLED"), default=True),
        base_url=_default_base_url(),
        api_prefix=_normalize_api_prefix(os.getenv("JARVIS_BACKEND_API_PREFIX")),
        timeout_seconds=_parse_float(os.getenv("TELEGRAM_BACKEND_TIMEOUT_SECONDS"), 95.0),
        connect_timeout_seconds=_parse_float(
            os.getenv("TELEGRAM_BACKEND_CONNECT_TIMEOUT_SECONDS"),
            2.5,
        ),
    )
...
class TelegramBackendClient:
    def __init__(self, config: TelegramBackendConfig | None = None) -> None:
    ...
    def send_message(
        ...
        *,
    ...
    ) -> TelegramBackendMessageResult:
        if not self.config.enabled:
            raise TelegramBackendError("Telegram backend klient je vypnutý.")
        ...
        if not payload["text"]:
            raise TelegramBackendError("Nelze odeslat prázdnou Telegram zprávu do backendu.")
        ...
        try:
            ...
            raise TelegramBackendError(f"Backend není dostupný: {exc}") from exc
        except ValueError as exc:
            raise TelegramBackendError("Backend vrátil neplatnou JSON odpověď.") from exc

        status = str(data.get("status", "") or "").strip()
        if not status:
            raise TelegramBackendError("Backend nevrátil stav odpovědi.")

        return TelegramBackendMessageResult(
            status=status,

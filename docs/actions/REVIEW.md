...
|---|---:|---|---|
| `003_open_app/` | ano | ověřeno s omezením | Otevírá aplikace přes aliasy, PATH, URI schémata a neblokující Windows `start` v samostatném okně. Úspěch u některých aplikací nelze spolehlivě ověřit. Popis nástroje je v `actions/tool_catalog.py`. |
